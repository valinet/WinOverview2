...
INT get_symbols(const char* pdb_file, DWORD* addresses)
{
    ...
    g_sym_count = 0;
    SymEnumSymbols(g_handle, base_addr, ADDR_STR0, enum_proc, NULL);
    if (g_sym_count != 1)
    {
        return -4;
    }
    SymEnumSymbols(g_handle, base_addr, ADDR_STR1, enum_proc, NULL);
    if (g_sym_count != 2)
    {
        return -5;
    }
    SymEnumSymbols(g_handle, base_addr, ADDR_STR2, enum_proc, NULL);
    if (g_sym_count != 3)
    {
        return -6;
    }
    SymEnumSymbols(g_handle, base_addr, ADDR_STR3, enum_proc, NULL);
    if (g_sym_count != 4)
    {
        return -7;
    }
    SymEnumSymbols(g_handle, base_addr, ADDR_STR4, enum_proc, NULL);
    if (g_sym_count != 5)
    {
        return -8;
    }
